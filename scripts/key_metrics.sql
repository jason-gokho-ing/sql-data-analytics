...
FROM gold.dim_customer_info ci
LEFT JOIN gold.fact_sales s ON ci.customer_id = s.customer_key
GROUP BY ci.customer_key
)
...
SELECT customer_segment, COUNT(customer_key) AS total_customers FROM
    ...
    SELECT customer_key, total_spending, customer_lifespan,
            CASE
...
-- 
SELECT cost_range, COUNT(*) [# of Customers] FROM (
...
SELECT product_number, product_name, cost,
    CASE
        ...
        ELSE 'Above 1000'
    END AS cost_range
FROM gold.dim_product_info pr
GROUP BY product_number, product_name, cost
) t
